...
#[tokio::test]
async fn status_command_shows_active_role_only_when_set() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    assert!(
        ...
        "default sessions should not render a Role line, got: {rendered}"
    );

    chat.config.role = Some("writer".to_string());
    chat.dispatch_command(SlashCommand::Status);
    let rendered = match rx.try_recv() {
        Ok(AppEvent::InsertHistoryCell(cell)) => {
            lines_to_single_string(&cell.display_lines(/*width*/ 80))
    ...
    assert!(
        rendered.contains("Role") && rendered.contains("writer"),
        "expected /status to render the active role, got: {rendered}"
    );
}
